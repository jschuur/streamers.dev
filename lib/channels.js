...
async function updateChannelDetails({
  ...
  const lastChannelState = await getChannels({
    fields: [
      ...new Set([...STATUS_UPDATE_FIELDS, ...PROFILE_CHANGE_FIELDS]),
      'country',
      'country2',
    ],
    includePaused,
...
export async function getPotentialChannels() {
  ...
  // Out of the coding streams, find out which are already in the DB
  ...
  const liveCodingUsers = await twitchGetUsersByIds(
    map(
      liveCodingStreams.filter(({ userId }) => !existingChannelTwitchIds.includes(userId)),
      'userId'
    )
  );
  ...
  // TODO: filter out channels are processed in the queue
