...
export async function getPotentialChannels() {
  ...
  const validgameDevTagIds = Object.values(gameDevTagIds);
  ...
  const liveCodingStreams = liveStreams.filter(
    (stream) =>
      ...
      !validgameDevTagIds.some((tagId) => stream.tagIds.includes(tagId))
  );
  ...
  return { channels: orderBy(potentialChannels, ['viewers'], ['desc']) };
}