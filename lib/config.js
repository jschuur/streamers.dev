...
export const OFFLINE_CHANNELS_STALE_MINUTES =
  parseInt(process.env.NEXT_PUBLIC_OFFLINE_CHANNELS_STALE_MINUTES) || 20;
export const STATS_DATA_STALE_MINUTES =
  parseInt(process.env.NEXT_PUBLIC_STATS_DATA_STALE_MINUTES) || 30;
export const EXPLORE_DATA_STALE_MINUTES =
  parseInt(process.env.NEXT_PUBLIC_EXPLORE_DATA_STALE_MINUTES) || 30;
export const EXPLORE_RECENT_STREAM_TOPICS_DAYS =
  parseInt(process.env.NEXT_PUBLIC_EXPLORE_RECENT_STREAM_TOPICS_DAYS) || 7;
...
export const twitchTagsToStreamTags = Object.keys(twitchTags).reduce((acc, key) => {
  const streamTag = twitchTags[key].streamTag;

  return streamTag ? [...acc, streamTag === true ? key : { [key]: streamTag }] : acc;
}, []);

// Twitch tabs specifically used by game development streams
export const gameCodingTagIds = Object.keys(twitchTags).reduce(
