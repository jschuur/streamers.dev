...
export async function addOrUpdateStream(channel) {
  ...
  // TODO: Merge in latestTags with recent stream (if same stream ID)
  ...
  try {
    const data = {
      ...
      gameName: channel.latestStreamGameName,
      language: channel.latestStreamLanguage,
      viewers: channel.latestStreamViewers,
      peakViewers: channel.latestStreamPeakViewers,
      twitchTags: channel.latestStreamTwitchTags || [],
      tags: channel.latestStreamTags || [],
      isCoding: channel.isCoding,
      isLive: channel.isLive,
    };

    if (channel.isLive) data.lastOnline = new Date();
...
export async function markStreamsOffline(channelIds) {
  try {
    ...
    logger.error(`Error marking streams offline: ${message}`);
  }
}
