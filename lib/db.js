...
export async function getOfflineChannels({ topic, lang }) {
  // TODO: Filter by coding, new users only
  const params = {
    where: {
      isLive: false,
      streams: {
  ...
  if (lang === 'english') params.where.streams.some.language = 'en';
  else if (lang === 'notenglish') params.where.streams.some.NOT = { language: 'en' };

  const channels = await prisma.channel.findMany(params);

  // Return a sorted list by the sum of recent session's peak viewers and extract relevant fields
  return {
    channels: orderBy(
      channels.map((channel) => ({
        ...pick(channel, CHANNEL_GRID_FIELDS),
        peakViewerSum: sumBy(channel.streams, 'peakViewers') || 0,
        streamCount: channel.streams?.length || 0,
      })),
      ['peakViewerSum'],
      ['desc']
    ).slice(0, OFFLINE_CHANNELS_LIMIT),
  };
}
