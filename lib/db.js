...
export async function saveUpdatedChannels({
  ...
  for (const channel of channels.filter(hasUpdated)) {
    const data = pick(channel, fields);

    logger.verbose(
      `Saving updates for ${channel.name} (Twitch ID ${
        channel.twitchId
      }, stream tags '${channel.latestStreamTags.join(', ')}')`
    );
    ...
    try {
      ...
      logger.error(
        `Error while saving channel update for ${channel.name} (Twitch ID ${
          channel.twitchId
        }, stream tags '${channel.latestStreamTags.join(', ')}') ${message}`
      );
  ...
  logger.info(
    `...${pluralize('updated channel', updatesCount, true)} saved${
      errorsCount > 0 ? `, ${pluralize('error', errorsCount, true)}` : ''
    }, ${pluralize('channel', offlineCount, true)} went offline`
  );
}
...
export async function getOfflineChannels({ topic, lang }) {
  ...
  const sql = `
    SELECT
      ...
      AVG(s."peakViewers") AS "peakViewerAverage",
