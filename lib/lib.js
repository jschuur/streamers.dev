...
export async function updateAllUserDetails() {
  return updateUserDetails();
}

// Only update the current status details for all users
export async function updateAllUserStatuses() {
  return updateUserDetails({ statusOnly: true });
}

async function updateUserDetails({ statusOnly = false } = {}) {
