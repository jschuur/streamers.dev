...
async function streamStats() {
  // TODO: Break down by coding and non coding
  // TODO: Another query to rewrite using either queryRaw or prisma.strean.gropBy when supported
  const streamsByDay = await prisma.$queryRawUnsafe(`
    SELECT
  ...
  const streamsByDaySeries = buildChartData(streamsByDay, 'count');

  return { streamsByDaySeries };
}
