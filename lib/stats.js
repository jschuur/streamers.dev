...
async function countryStats() {
  ...
  const countryData = groupBy(
    flatMap(
      results.map(({ countries, latestStreamViewers: viewers }) =>
  ...
  console.log(countryData['us'].length);
  const countriesByStreams = orderBy(
    Object.keys(countryData).map((countryCode) => ({
      ...
      count: countryData[countryCode].length,
    })),
    ['count'],
    ['desc']
  );
  const countriesByViewers = orderBy(
    Object.keys(countryData).map((countryCode) => ({
      name:
        countries.getName(countryCode.toUpperCase(), 'en', { select: 'official' }) || countryCode,
      viewers: sumBy(countryData[countryCode], 'viewers'),
    })),
    ['viewers'],
    ['desc']
  );
