...
import { union, partition, concat } from 'lodash';

import { getKeywords } from './db';
import logger from './logger';
import { debug } from './util';

import { twitchTagsToStreamTags } from './config';
...
export function findTags({ str, twitchTags }) {
  if (!str?.length) return [];
  ...
  const fullKeywordMatches = fullMatchKeywords
    .filter(({ keywords }) =>
      keywords.some((keyword) => {
        const match = str.toLowerCase().indexOf(keyword.toLowerCase()) >= 0;
        if (match) logger.verbose(`Found full match '${keyword}' in '${str}'`);
  ...
  // ...map some Twitch tags to stream topic tags too...
  const twitchTagMatches = Object.entries(twitchTagsToStreamTags).reduce(
    (acc, [twitchTag, streamTag]) =>
      twitchTags.includes(twitchTag) ? concat(acc, streamTag) : acc,
    []
  );

  debug(
    JSON.stringify(
      { fullKeywordMatches, fragmentKeywordMatches, fragmentRegexpMatches, twitchTagMatches },
      null,
      2
    )
  );
  ...
  const streamTags = union(
    fullKeywordMatches,
    fragmentKeywordMatches,
    fragmentRegexpMatches,
    twitchTagMatches
  );
...
export async function addStreamTags(channels) {
  ...
  const [liveCodingChannels, otherChannels] = partition(channels, { isLive: true, isCoding: true });
  ...
  liveCodingChannels.forEach((channel) => {
    const { latestStreamTitle, allStreamTags = [], latestStreamTwitchTags = [] } = channel;
    const currentTags = findTags({
      str: latestStreamTitle.trim(),
      twitchTags: latestStreamTwitchTags,
    });

    // Add any new tags to the old list, so we also handle stream title changes
    channel.allStreamTags = union(allStreamTags, currentTags);
    channel.latestStreamTags = currentTags;
  });

  // Clear the topic tags for non-coding channels
  otherChannels.forEach((channel) => (channel.latestStreamTags = []));
