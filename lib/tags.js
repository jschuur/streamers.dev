...
import { union, partition } from 'lodash';
...
export function findTags({ str, twitchTags }) {
  ...
  const twitchTagMatches = twitchTagsToStreamTags.reduce((acc, tag) => {
    const [twitchTag, streamTag] = typeof tag === 'string' ? [tag, tag] : [tag[0], tag[1]];

    return twitchTags.includes(twitchTag) ? acc.concat(streamTag) : acc;
  }, []);
