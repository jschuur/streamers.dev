...
import { twitchGetStreamsByIds, twitchGetUsersByIds, twitchGetUsersByNames } from './twitch_api';
...
export async function getChannelInfoFromTwitch({ userIds, userNames }) {
  ...
  // Add details from the current stream for live channels
  await updateChannelStatuses({ channels, updateAll: true });
...
export async function updateChannelStatuses({ channels, updateAll = false }) {
  ...
  // Find outdated channel entries (always update live channels if MAX_STATUS_AGE_SECONDS is set)
  ...
  if (channelsToUpdate?.length) {
    console.log(`Refreshing status for ${pluralize('channel', channelsToUpdate.length, true)}...`);

    const liveStreams = await twitchGetStreamsByIds(map(channelsToUpdate, 'twitchId'));
    console.log(`${pluralize('channel', liveStreams.length, true)} of those live.`);
