...
export async function updateUserStatuses({ users, updateAll = false }) {
  ...
  if (usersToUpdate?.length) {
    ...
    const liveStreams = await twitchGetStreamsByIds(map(usersToUpdate, 'twitchId'));
    ...
    usersToUpdate.forEach((user) => {
      ...
      if (stream) {
        ...
        user.latestStreamLanguage = stream.language;
        user.latestStreamGameName = stream.gameName;
